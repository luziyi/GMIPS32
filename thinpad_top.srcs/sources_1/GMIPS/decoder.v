/*
 * File:         e:\NSCSCC\GMIPS\thinpad_top.srcs\sources_1\GMIPS\decoder
 * Project:      e:\NSCSCC\GMIPS\thinpad_top.srcs\sources_1\GMIPS
 * Created Date: 2024-07-17 16:01:00
 * Author:       Tommy Gong
 * description:  
 * ----------------------------------------------------
 * Last Modified: 2024-07-17 16:01:35
 */
 `include "defines.v"
module decoder (
  ...
  /*J-TYPE*/
  wire J = (op == 6'b000010), JAL = (op == 6'b000011);
  /*load&save*/
  wire LW = (op == 6'b100011), LB = (op == 6'b100000), SW = (op == 6'b101011), SB = (op == 6'b101000);
  /*空指令*/
  wire NOP = 32'b0;
  ...
  assign RegWriteEn = !NOP && (ADD || ADDU || SUB || SLT || MUL || AND || OR || XOR || SLL || SLLV || SRA || SRAV || SRL || SRLV || JALR || ADDI || ADDIU || ANDI || LUI || ORI || XORI || LW || LB || JAL);
  assign RegDst     = (ADD || ADDU || SUB || SLT || MUL || AND || OR || XOR || SLL || SLLV || SRA || SRAV || SRL || SRLV || (JALR && RdID)) ? 2'b00 : (ADDI || ADDIU || ANDI || LUI || ORI || XORI) ? 2'b01 : JAL || (JALR && !RdID) ? 2'b10 : 2'b11;  //2'b11时行为未定义
